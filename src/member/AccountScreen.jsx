  ...
  if (!user) {
    return (
      <div className="pf-scroll pf-screen flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
        ...
        <button onClick={() => navigate('/login')} style={{ fontSize: 13.5, color: 'var(--color-accent-700)' }}>Sign in</button>
        <div style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid var(--color-divider)', width: '100%' }}>
          <p style={{ fontSize: 12.5, color: 'color-mix(in srgb,var(--color-text) 50%,transparent)', marginBottom: 8 }}>Run a church, or work for PewFinder?</p>
          ...
          <button onClick={() => navigate('/staff')} style={{ fontSize: 13.5, color: 'var(--color-accent-700)', display: 'block', margin: '0 auto' }}>PewFinder staff sign-in &rarr;</button>
        </div>
      </div>
    )
  }
