...
export function AccountScreen() {
  ...
  if (!user) {
    return (
      <div className="pf-scroll pf-screen flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
        ...
        <button onClick={() => navigate('/login')} style={{ fontSize: 12.5, color: 'var(--color-accent-700)' }}>Sign in</button>
        <div style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid var(--color-divider)', width: '100%' }}>
          <p style={{ fontSize: 11.5, color: 'color-mix(in srgb,var(--color-text) 50%,transparent)', marginBottom: 8 }}>Run a church, or work for PewFinder?</p>
          <button onClick={() => navigate('/admin')} style={{ fontSize: 12.5, color: 'var(--color-accent-700)', display: 'block', margin: '0 auto 6px' }}>Claim your church &rarr;</button>
          <button onClick={() => navigate('/staff')} style={{ fontSize: 12.5, color: 'var(--color-accent-700)', display: 'block', margin: '0 auto' }}>PewFinder staff sign-in &rarr;</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="pf-scroll pf-screen flex-1" style={{ padding: '60px 20px 8px' }}>
      ...
      <p style={{ fontSize: 13, color: 'color-mix(in srgb,var(--color-text) 60%,transparent)', marginTop: 6 }}>{profile?.name} · {user.email}</p>
      ...
      <button onClick={() => navigate('/staff')} className="btn btn-secondary" style={{ marginTop: 10, padding: '11px 15px', display: 'block', width: '100%', textAlign: 'left' }}>
        {staffRole ? 'PewFinder staff portal' : 'PewFinder staff sign-in'}
      </button>
