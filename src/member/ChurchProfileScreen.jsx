  ...
  return (
    <div className="pf-scroll pf-screen flex-1" style={{ padding: '0 0 8px' }}>
      <div className="flex items-center gap-3 px-3" style={{ position: 'sticky', top: 0, zIndex: 2, height: 48, background: 'color-mix(in srgb,var(--color-surface) 94%,transparent)', backdropFilter: 'blur(6px)', borderBottom: '1px solid var(--color-divider)' }}>
        ...
        <span className="pf-h flex-1 min-w-0 truncate" style={{ fontSize: 15 }}>{church.name}</span>
        ...
        <div className="flex items-center gap-2" style={{ marginTop: 11, color: 'var(--color-accent-2)' }}>
          {church.rated ? (
            <span className="flex items-center gap-2">
              ...
              <span style={{ fontSize: 12.5, color: 'color-mix(in srgb,var(--color-text) 55%,transparent)' }}>{church.review_count} reviews</span>
            </span>
          ) : (
            <span style={{ fontSize: 12.5, color: 'var(--color-accent-600)' }}>Unclaimed listing · no reviews yet</span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-[7px] px-5" style={{ paddingTop: 15 }}>
        {church.facts.map((t, i) => (
          <span key={i} className="flex-none border rounded-[var(--radius-md)]" style={{ padding: '5px 9px', fontSize: 12, borderColor: 'var(--color-divider)', color: 'color-mix(in srgb,var(--color-text) 68%,transparent)' }}>{t}</span>
        ))}
      </div>
        ...
        <>
          <div className="px-5" style={{ paddingTop: 26 }}>
            <div className="flex items-baseline justify-between">
              ...
              <span style={{ fontSize: 11, letterSpacing: '.09em', textTransform: 'uppercase', color: 'color-mix(in srgb,var(--color-text) 45%,transparent)' }}>Ranked</span>
            </div>
          </div>
          ...
          {middle.length > 0 && (
            <>
              <div className="px-5" style={{ paddingTop: 16 }}>
                <button onClick={() => setShowAllCats((v) => !v)} className="flex items-center gap-[6px]" style={{ fontSize: 13, color: 'var(--color-accent-600)' }}>
                ...
                <div className="px-5 flex flex-col gap-[9px]" style={{ paddingTop: 14 }}>
                  {middle.map((s) => (
                    <div key={s.key} className="flex items-center justify-between gap-[10px] border-b" style={{ paddingBottom: 8, borderColor: 'color-mix(in srgb,var(--color-text) 10%,transparent)' }}>
                      <span style={{ fontSize: 13.5, color: 'color-mix(in srgb,var(--color-text) 78%,transparent)' }}>{s.label}</span>
                      <span className="flex items-center gap-[7px]" style={{ color: 'var(--color-accent-2)' }}>
                        <StarRow value={s.score} size={11} />
                        <span style={{ fontSize: 12.5, color: 'color-mix(in srgb,var(--color-text) 60%,transparent)' }}>{s.score.toFixed(1)}</span>
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex gap-[18px] border-b px-5" style={{ marginTop: 26, borderColor: 'var(--color-divider)' }}>
            {[['reviews', 'Reviews'], ['notes', 'Sermon notes']].map(([id, label]) => (
              <button key={id} onClick={() => setTab(id)} style={{ fontSize: 13, letterSpacing: '.05em', textTransform: 'uppercase', padding: '9px 0', borderBottom: `2px solid ${tab === id ? 'var(--color-accent)' : 'transparent'}`, color: tab === id ? 'var(--color-text)' : 'color-mix(in srgb,var(--color-text) 48%,transparent)' }}>
                {label}
              </button>
            ))}
          </div>
          ...
          {tab === 'notes' && (
            <div style={{ paddingTop: 6 }}>
              {notes.map((n) => (
                <div key={n.id} className="px-5 border-b" style={{ padding: '16px 20px', borderColor: 'var(--color-divider)' }}>
                  <div className="flex items-baseline justify-between gap-[10px]">
                    ...
                    <span style={{ fontSize: 11.5, color: 'color-mix(in srgb,var(--color-text) 48%,transparent)', whiteSpace: 'nowrap' }}>{n.date_preached}</span>
                  </div>
                  <div style={{ fontSize: 12, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--color-accent-600)', marginTop: 6 }}>{n.scripture}</div>
                  <p style={{ margin: '9px 0 0', fontSize: 13.5, lineHeight: 1.62, color: 'color-mix(in srgb,var(--color-text) 80%,transparent)' }}>{n.summary}</p>
                  {n.media_link && (
                    <a href={n.media_link} target="_blank" rel="noreferrer" className="flex items-center gap-[6px]" style={{ marginTop: 11, color: 'var(--color-accent-700)', fontSize: 12.5 }}>
              ...
              {notes.length === 0 && (
                <div className="px-5" style={{ padding: '26px 20px', fontSize: 13.5, lineHeight: 1.62, color: 'color-mix(in srgb,var(--color-text) 55%,transparent)' }}>
              ...
              {reviews.map((r) => (
                <div key={r.id} className="px-5 border-t" style={{ marginTop: 14, padding: '15px 20px 16px', borderColor: 'var(--color-divider)' }}>
                  <div className="flex items-center gap-[10px]">
                    <div className="flex-none rounded-full border flex items-center justify-center" style={{ width: 34, height: 34, borderColor: 'var(--color-divider)', background: avatarColor(r.author_initials), fontFamily: 'var(--font-heading)', fontSize: 15, color: 'var(--color-surface)' }}>
                      {r.author_initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div style={{ fontSize: 13.5, fontWeight: 600 }}>{r.author_display_name}</div>
                      <div style={{ fontSize: 11.5, color: 'color-mix(in srgb,var(--color-text) 52%,transparent)' }}>Visited {r.visited_on}</div>
                    </div>
                    <span style={{ flex: 'none', color: 'var(--color-accent-2)' }}><StarRow value={r.overall_rating} size={12} /></span>
                  </div>
                  {r.well_text && <p style={{ margin: '10px 0 0', fontSize: 13.5, lineHeight: 1.62, color: 'color-mix(in srgb,var(--color-text) 82%,transparent)' }}>{r.well_text}</p>}
                  {r.tags?.length > 0 && (
                  ...
                  {r.reply && (
                    <div style={{ marginTop: 12, paddingLeft: 12, borderLeft: '1px solid var(--color-accent)' }}>
                      <div className="flex items-center gap-[5px]" style={{ fontSize: 11, letterSpacing: '.09em', textTransform: 'uppercase', color: 'var(--color-accent-600)' }}>
                        <BadgeCheck size={12} strokeWidth={1.6} /><span>Response from {church.name}</span>
                      </div>
                      <p style={{ margin: '6px 0 0', fontSize: 13, lineHeight: 1.6, fontStyle: 'italic', color: 'color-mix(in srgb,var(--color-text) 72%,transparent)' }}>{r.reply}</p>
                    </div>
                  )}
      ...
      {!church.claimed && (
        <div className="flex items-start gap-[10px] border-t px-5" style={{ margin: '26px 20px 0', paddingTop: 16, borderColor: 'var(--color-divider)' }}>
          ...
          <div className="flex-1 min-w-0">
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Is this your church?</div>
            <p style={{ margin: '5px 0 0', fontSize: 12.5, lineHeight: 1.6, color: 'color-mix(in srgb,var(--color-text) 60%,transparent)' }}>
              Claiming is free. Reply to reviews, fix your service times, keep your programs current.
            </p>
            <button onClick={() => navigate('/admin')} className="border" style={{ marginTop: 9, fontSize: 12.5, padding: '8px 13px', borderColor: 'var(--color-accent)', color: 'var(--color-accent-700)' }}>
              Claim this page
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-[10px]">
        ...
        <span style={{ fontSize: 13, color: accent ? 'var(--color-accent-700)' : 'color-mix(in srgb,var(--color-text) 60%,transparent)' }}>{s.score.toFixed(1)}</span>
      </div>
