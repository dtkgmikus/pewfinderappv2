...
export function FeedScreen() {
  ...
  return (
    <div className="pf-scroll pf-screen flex-1" style={{ padding: '0 0 8px' }}>
      <div className="px-5 border-b" style={{ position: 'sticky', top: 0, zIndex: 2, paddingTop: 24, paddingBottom: 16, background: 'var(--color-bg)', borderColor: 'var(--color-divider)' }}>
        <div className="flex items-center gap-[5px]" style={{ color: 'var(--color-accent-600)', fontSize: 10.5, letterSpacing: '.1em', textTransform: 'uppercase' }}>
        ...
        <p style={{ margin: '8px 0 0', fontSize: 12, color: 'color-mix(in srgb,var(--color-text) 58%,transparent)' }}>Newest reviews from churches around Egg Harbor Township and Mays Landing.</p>
      </div>

      {items.map((r) => (
        <div key={r.id} className="px-5 border-b" style={{ padding: '16px 20px', borderColor: 'var(--color-divider)' }}>
          <div className="flex items-center gap-[10px]">
            <div className="flex-none rounded-full border flex items-center justify-center" style={{ width: 32, height: 32, borderColor: 'var(--color-divider)', background: avatarColor(r.author_initials), fontFamily: 'var(--font-heading)', fontSize: 13, color: 'var(--color-surface)' }}>
              {r.author_initials}
            </div>
            <div className="flex-1 min-w-0">
              <div style={{ fontSize: 12 }}><span style={{ fontWeight: 600 }}>{r.author_display_name}</span> reviewed</div>
          ...
          {r.well_text && <p style={{ margin: '9px 0 0', fontSize: 12.5, lineHeight: 1.62, color: 'color-mix(in srgb,var(--color-text) 82%,transparent)' }}>{r.well_text}</p>}
          {r.tags?.length > 0 && (
