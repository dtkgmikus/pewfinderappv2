    ...
    <div className="flex flex-col gap-5 max-w-[720px]">
      ...
      <div className="grid gap-4 items-start border" style={{ gridTemplateColumns: 'auto 1fr', borderColor: 'var(--color-divider)', padding: '20px 22px', background: 'var(--color-bg)' }}>
        ...
        <span className="flex flex-col gap-[6px]">
          ...
          <span style={{ fontSize: 13, lineHeight: 1.6, color: 'color-mix(in srgb,var(--color-text) 62%,transparent)' }}>
            {church.verified ? `Confirmed ${new Date(church.verified_at).toLocaleDateString()}.` : 'Contact Get-God staff to complete verification.'} Members see a plain &ldquo;Verified&rdquo; mark on your page — the same one every claimed church gets, whether or not they pay.
          </span>
        </span>
      </div>
      <div className="flex flex-col gap-3 border" style={{ borderColor: 'var(--color-divider)', padding: '20px 22px' }}>
        ...
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.62, color: 'color-mix(in srgb,var(--color-text) 68%,transparent)' }}>
          If the pastor or office administrator changes, the current Owner should add the new person in Team and hand over ownership. If nobody has access any more, contact Get-God staff to re-verify.
        </p>
        {role === 'owner' && (
