...
const FREE_FEATURES = ['Reply to reviews', 'Flag reviews that break the rules', 'Edit your profile & service times', 'Programs checklist']
const PRO_FEATURES = ['Everything in Free', 'Sermon notes with member notifications', 'Photos, video, and program links', 'Promoted events with geographic targeting', 'Insights — who is looking and what they want', 'Board & monthly reports']

export function BillingScreen() {
  const { church, subscription, isPro, role, refresh } = useAdmin()
  ...
  const togglePlan = async () => {
    if (!canManage) return
    if (isPro) {
      await supabase.from('subscriptions').upsert({ church_id: church.id, plan: 'free', status: 'active', started_at: new Date().toISOString() })
      await supabase.from('churches').update({ plan: 'free' }).eq('id', church.id)
    } else {
    ...
    <div className="flex flex-col gap-[22px] max-w-[820px]">
      <div>
        ...
        <p style={{ margin: '7px 0 0', fontSize: 13.5, color: 'color-mix(in srgb,var(--color-text) 60%,transparent)' }}>
          {isPro ? `On PewFinder Pro since ${new Date(subscription.started_at).toLocaleDateString()}.` : "You're on the free tier — claimed and verified, no card on file."}
        </p>
      </div>
      <div className="grid grid-cols-2 gap-4 items-start">
        ...
        <div className="border flex flex-col gap-[14px]" style={{ borderColor: 'var(--color-accent-500)', padding: 22, background: 'color-mix(in srgb,var(--color-accent) 6%,transparent)' }}>
          <div className="flex items-baseline justify-between gap-[10px]">
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 19 }}>PewFinder Pro</span>
          ...
          <button onClick={togglePlan} disabled={!canManage} className="border" style={{ background: isPro ? 'transparent' : 'var(--color-accent-700)', color: isPro ? 'var(--color-accent-800)' : 'var(--color-surface)', borderColor: 'var(--color-accent-700)', padding: '12px 16px', fontSize: 13.5, marginTop: 4 }}>
